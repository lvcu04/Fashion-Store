  ...
  const handleAddToCart = () => {
    ...
    UIManager.measure(
      ...
      (x, y, width, height, pageX, pageY) => {
        ...
        runOnJS(setShouldRenderImage)(true);
        ...
        UIManager.measure(
          ...
          (x2, y2, w2, h2, pageX2, pageY2) => {
            ...
            scale.value = withTiming(0.3, { duration: 500 }, () => {
              ...
              runOnJS(setShouldRenderImage)(false);
            });
          }
        );
      }
  ...
  return (
    <>
      ...
      <ScrollView className="flex-1 bg-white">
        {/* Product Image */}
        <View className="p-5 mt-10" ref={productImageRef}>
          <Image
        ...
        {/* Product Info */}
        <View className="px-5 rounded-t-3xl bg-white">
          ...
          <View className="flex-row justify-between items-center mt-2">
            ...
            <Text
              className={`font-semibold ${
                data.status === 'Stock' ? 'text-green-600' : 'text-red-500'
              }`}
            >
              {data.status}
            </Text>
          ...
          <View className="flex-row flex-wrap mt-2">
            {sizes.map((size) => (
              ...
              >
                <Text className={`${selectedSize === size ? 'text-white' : ''} text-base font-medium`}>
                  {size}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <View className="flex-row items-center px-4 py-4 border-t border-gray-200">
        <TouchableOpacity
          className="flex-1 bg-black rounded-full py-4 mr-3 mb-5"
      ...
      {/* Comment Modal */}
      <Modal animationType="slide" transparent visible={showComments} onRequestClose={() => setShowComments(false)}>
        <TouchableWithoutFeedback onPress={() => setShowComments(false)}>
          <View className="flex-1 justify-end bg-black/40">
            <TouchableWithoutFeedback>
              <View className="bg-white rounded-t-3xl px-5 pt-4 pb-6" style={{ height: '80%' }}>
                <Text className="text-lg font-bold mb-4 text-center">Reviews Product</Text>
                <ScrollView className="mb-4 flex-1" keyboardShouldPersistTaps="handled">
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={180}>
